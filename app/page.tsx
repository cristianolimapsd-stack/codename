    ...
    <main className="min-h-screen bg-[#1a1a2e] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg relative z-10">
        <div className="text-center mb-8">
          <h1
            className="text-7xl font-black tracking-tight leading-none mb-2"
            style={{ fontFamily: 'Georgia,serif', textShadow: '3px 3px 0px rgba(0,0,0,0.4)' }}
          >
            <span className="text-white">CODE</span>
            <span className="text-[#e8534a]">NAMES</span>
          </h1>
          <p className="text-[#6666aa] text-xs tracking-[0.4em] uppercase font-bold">
        ...
        <div className="flex bg-[#0d0d20] rounded-2xl p-1.5 border-2 border-[#2a2a4a] mb-1">
          <button
            ...
            className={`flex-1 py-3 rounded-xl font-black text-sm uppercase tracking-widest transition-all ${
              tab === 'create' ? 'bg-[#e8534a] text-white' : 'text-[#6666aa] hover:text-white'
            }`}
            style={{ fontFamily: 'Georgia,serif' }}
          >
            Criar Sala
          ...
          <button
            onClick={() => setTab('join')}
            className={`flex-1 py-3 rounded-xl font-black text-sm uppercase tracking-widest transition-all ${
              tab === 'join' ? 'bg-[#4a90d9] text-white' : 'text-[#6666aa] hover:text-white'
            }`}
            style={{ fontFamily: 'Georgia,serif' }}
          >
            Entrar
        ...
        {tab === 'create' && (
          <div className="bg-[#0d0d20] border-2 border-[#2a2a4a] rounded-2xl p-5">
            <p className="text-xs uppercase tracking-widest text-[#6666aa] mb-3 font-black">
              Pacote de Palavras
            </p>
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border-2 text-left transition-all ${
                    themes.includes(theme.id)
                      ? 'border-[#e8534a] bg-[#e8534a]/10'
                      : 'border-[#2a2a4a] hover:border-[#44446a]'
                  }`}
                >
                  <div className="font-bold text-sm text-white">{theme.label}</div>
                  <div className="text-xs text-[#6666aa] mt-0.5">{theme.desc}</div>
                </button>
              ))}
            </div>
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl border-2 border-dashed text-sm transition-all mb-3 text-left flex items-center gap-3 ${
                showCustom
                  ? 'border-[#e8534a] text-white bg-[#e8534a]/5'
                  : 'border-[#2a2a4a] text-[#6666aa] hover:border-[#44446a] hover:text-white'
              }`}
            >
        ...
        <p className="text-center text-[#3a3a5a] text-xs mt-5">
